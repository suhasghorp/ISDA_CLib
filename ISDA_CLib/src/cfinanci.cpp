     ...
     double          *rate)           /* (O) output rate */
{
    ...
    if (discount <= 0.0)
    {
		char msg[] = "%s: Bad discount factor %f.\n";
        //JpmcdsErrMsg ("%s: Bad discount factor %f.\n",routine,discount) ;
		JpmcdsErrMsg(msg, routine, discount);
        goto done;
    }
    ...
    if (rateBasis == JPMCDS_DISCOUNT_FACTOR)
    {
        if (startDate == endDate)
        {
            ...
            {
				char msg[] = "%s: Start date (%s) equals end date (%s), "
    ...
    if (startDate == endDate)
	{
		char msg[] = "%s: Start date %s cannot equal end date.\n";
        JpmcdsErrMsg(msg,
                  routine, 
                  JpmcdsFormatDate(startDate));
        goto done;
     ...
     double        *discount)         /* (O) Discount factor */
{
    ...
    if (rateBasis == JPMCDS_DISCOUNT_FACTOR)
    {
       if (rate <= 0.0)
       {
		   char msg[] = "%s: Bad rate (discount factor) %f\n";
           JpmcdsErrMsg(msg,routine,rate);
           goto done;
       }
       *discount = rate;
       return SUCCESS;
    ...
    {
		char msg[] = "%s: Basis (%ld) < %d.\n";
        JpmcdsErrMsg (msg,routine,rateBasis,
                   JPMCDS_SIMPLE_BASIS);
        goto done;                      /* Failed */
    }
    ...
    double *rate )                     /* (O) Output rate */
{
    ...
    case JPMCDS_SIMPLE_BASIS:
        ...
        {
			char msg[] = "%s: Bad discount:%f or year fraction:%f\n";
            JpmcdsErrMsg(msg,
    ...
    case JPMCDS_DISCOUNT_RATE:
        if (IS_ALMOST_ZERO(yearFraction))
        {
			char msg[] = "%s: Undefined discount rate\n";
            JpmcdsErrMsg(msg,routine);
            *rate = 0.0;
            return FAILURE;
        }
    ...
    case JPMCDS_CONTINUOUS_BASIS:
        if (IS_ALMOST_ZERO(yearFraction))
        {
			char msg[] = "%s: Div by zero - yr frac %f.\n";
            JpmcdsErrMsg(msg,
    ...
    case JPMCDS_SIMPLE_BASIS:
        {
            ...
            {
				char msg[] = "%s: Invalid simple interest rate:%f\n";
                JpmcdsErrMsg(msg,
                          routine, rate);
                *discount = 0.0;
                goto done;
    ...
    case JPMCDS_DISCOUNT_RATE:
        ...
        else
        {
            *discount = 1.0 - rate * yearFraction;
            if (*discount <= 0.0)
            {
				char msg[] = "%s: Invalid discount rate:%f\n";
                JpmcdsErrMsg(msg,
                          routine, rate);
                *discount = 0.0;
                goto done;
    ...
    double basis)             /* (I) Compounding basis */
{
    ...
    case JPMCDS_SIMPLE_BASIS:
        ...
        {
			char msg[] = "%s: Simple Rate (%f) * Year Fraction (%f) must be > -1.0.\n" ;
            JpmcdsErrMsg(msg,
    ...
    case JPMCDS_DISCOUNT_RATE:
        if (rate * yearFraction >= 1.0)
        {
			char msg[] = "%s: Discount Rate (%f) * Year Fraction (%f) must be < 1.0.\n";
            JpmcdsErrMsg(msg,
                      routine, rate, yearFraction);
            goto done;
        }
    ...
    case JPMCDS_DISCOUNT_FACTOR:
        if (rate <= 0.0)
        {
			char msg[] = "%s: Discount factor (%f) must be > 0.0.\n";
            JpmcdsErrMsg(msg,
                      routine, rate);
            goto done;
        }
    ...
    default:
        if (rate <= -basis)
        {
			char msg[] = "%s: Rate (%f) must be greater than -basis (%f).\n";
            JpmcdsErrMsg(msg,
                      routine, rate, -basis);
            goto done;
        }
